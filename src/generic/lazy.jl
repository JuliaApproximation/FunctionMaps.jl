...
isrealmap(m::SimpleLazyMap) = isrealmap(supermap(m))
isrealmap(m::CompositeLazyMap) = all(map(isrealmap, components(m)))
...
map_hash(m::WrappedMap, h::UInt) = hashrec("WrappedMap", supermap(m), h)
